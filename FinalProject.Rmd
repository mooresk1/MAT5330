...
for (k in 1:t) { # for t trials
  ...
  trialmaxcombo[k] <- max(cand[i,3]) # max combo rate + skills this trial
  trialmeancombo[k] <- mean(as.numeric(cand[i,3])) # mean combo this trial
} 
trialmaxrating[1:16]
trialmaxskills[1:16]
trialmaxcombo[1:16]
trialmeanrating[1:16]
trialmeanskills[1:16]
trialmeancombo[1:16]
...
mean(as.numeric(trialmaxcombo)) # mean of max skills all trials
max(trialmaxcombo) # max skills over all trials
...
gtms <- ggplot(data=sdf, aes(x=sdf$trialmeanskills,fill="blue")) + 
  geom_histogram(binwidth=1, colour = "black", alpha = 0.3, position="dodge") +
...
gtmc <- ggplot(data=cdf, aes(x=cdf$trialmeancombo,fill="blue")) + 
  ...
  labs(x = "Mean Rating + Skills", y="Count", title = "Mean Rating + Skill Combo by Trial") +
