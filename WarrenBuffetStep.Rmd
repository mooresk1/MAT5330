...
rdata <- matrix(nrow = 100^3, ncol = 4) 
```

```{r}
# Loops: Outer to Inner --> each ratio (k), for n trials (j), for 20 years (i)
for (k in 1:100) {
  perR <- as.numeric(1 - 0.01*k)
  perY <- as.numeric(1 - perR)
  perG <- as.numeric(1 - perR - perY)
  ...
  rdata[k,1] <- as.numeric(perR)
  rdata[k,2] <- as.numeric(perY)
  rdata[k,3] <- as.numeric(perG)
